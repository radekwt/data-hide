...
file_path_partition_offset=$file_path
...
if [ ! -e "$file_to_hide" ] || [ ! -e "$partition" ] || [ ! -e "$file_path_partition_offset" ]; then
    echo "FILE NOT FOUND"
    exit
fi 
...
sync && ./info "$file_path_partition_offset" "$file_to_hide" "$partition" "$((block_size/512))" "512" "$last_cluster_index"
exit
fi
echo "FAT"
#IF PARTITION IS FAT
...
sync && sudo dd if="$partition" bs=$bytes_per_cluster skip=$root_dir_cluster count=1  | xxd -p -c "32" > dir.txt
sync && sudo dd if="$partition" bs=$bytes_per_cluster skip=$root_dir_cluster count=1  | xxd  > dir.txt.readable

sync && ./getFileCluster.sh "$root_dir_cluster" "$bytes_per_cluster" "$file_path" "$partition"
sync && ./findLastCluster.sh "$root_dir_cluster" "$partition" "$root_clus" "$number_of_reserved_sectors" "$bytes_per_sector"
...
sync && ./info "$file_path_partition_offset" "$file_to_hide" "$partition" "$sectors_per_cluster" "$bytes_per_sector" "$last_cluster_index"